...
const Home = () => {
    ...
    const getAppendedHTML = (slice) => {
        ...
        slice.map((eachCourse, index) => <Paper elevation={4} className={classes.detailCard} key={index}>
                    ...
                    <div id={"uniName-" + index}><a href={eachCourse.crawledCourses.website} target="_blank" rel="link to uni site"><span>{eachCourse.crawledCourses.uniName}</span></a>
                    </div>
                </Paper>)
                )
    }
    ...
    return (
        <Container maxWidth="xl" style={{ padding: "20px" }}>
            ...
            <Box mt={2} style={{textAlign:'center'}}>
                ...
                <Grid item sm={12} md={4} lg={3} order={{ xs: 1, sm: 1 }}>
